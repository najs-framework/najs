...
describe('Cookie', function() {
  ...
  describe('constructor()', function() {
    ...
    it('gets cookies and signedCookies if the controller is ExpressController, merged 2 variables to data', function() {
      ...
      const controller = Reflect.construct(ExpressController, [request])
      const cookie = new Cookie(controller)
  ...
  describe('.forget()', function() {
    it('is chain-able', function() {
      ...
      const cookie = new Cookie(<any>{ response })
  ...
  describe('.make()', function() {
    ...
    it('calls Response.cookie() with secure in option if secure is provided', function() {
      ...
      expect(
        cookieSpy.calledWith('name', 'value', { signed: true, path: '/', domain: 'najs.com', secure: false })
      ).toBe(true)

      cookie.make('name', 'value', true, 0, '/', 'najs.com', true)
      expect(cookieSpy.calledWith('name', 'value', { signed: true, path: '/', domain: 'najs.com', secure: true })).toBe(
        true
      )
      cookieSpy.restore()
    })
    ...
    it('calls Response.cookie() with httpOnly in option if httpOnly is provided', function() {
      ...
      expect(
        cookieSpy.calledWith('name', 'value', {
          signed: true,
          path: '/',
          domain: 'najs.com',
          secure: false,
